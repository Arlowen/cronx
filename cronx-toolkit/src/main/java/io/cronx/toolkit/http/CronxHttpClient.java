...
import io.cronx.toolkit.model.ResultEnum;
import io.cronx.toolkit.utils.ExceptionUtils;
import lombok.extern.slf4j.Slf4j;
import okhttp3.*;

@Slf4j
public class CronxHttpClient {
    ...
    public static ResponseData post(String url, String json, Map<String, String> headerMap) {
        ...
        try {
            ...
            if (response.code() == 200) {
                ResponseData successData = new ObjectMapper().readValue(response.body().string(), ResponseData.class);
                ...
                log.error(errorMsg);
                return ResponseDataUtil.buildError(ResultEnum.ERROR.getCode(), "error", response.body().string());
            }
        } catch (Throwable e) {
    ...
    public static ResponseData getWithJsonDeserialization(String url) {
        ResponseData rs = getWithResponseData(url, new HashMap<>());
    ...
    public static Response getWithBasicAuth(String url, Map<String, String> headers, String userName, String password) {
