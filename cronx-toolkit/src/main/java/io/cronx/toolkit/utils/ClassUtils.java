 ...
 * </p>
 * <p>
 * The notion of a <code>canonical name</code> includes the human readable name for the type, for example <code>int[]</code>. The non-canonical method variants work with the JVM names, such as
 * <code>[I</code>.
 * </p>
 *
...
public class ClassUtils {
     ...
     * </p>
     * <p>
     * The order is determined by looking through each interface in turn as declared in the source file and following its hierarchy up. Then each superclass is considered in the same way. Later
     ...
     * </p>
     * <p>
     * A new <code>List</code> is returned. If the class name cannot be found, <code>null</code> is stored in the <code>List</code>. If the class name in the <code>List</code> is <code>null</code>,
     ...
     * </p>
     * <p>
     * This method calls {@link #isAssignable(Class, Class) isAssignable} for each Class pair in the input arrays. It can be used to check if a set of arguments (the first parameter) are suitably
     * compatible with a set of method parameter types (the second parameter).
     * </p>
     * <p>
     * Unlike the {@link Class#isAssignableFrom(Class)} method, this method takes into account widenings of primitive classes and <code>null</code>s.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Specifically, this method tests whether the type represented by the specified <code>Class</code> parameter can be converted to the type represented by this <code>Class</code> object via an
     * identity conversion widening primitive or widening reference conversion. See <em><a href="http://java.sun.com/docs/books/jls/">The Java Language Specification</a></em>, sections 5.1.1, 5.1.2
     ...
     * @return an array which contains for each given class, the wrapper class or the original class if class is not a primitive. <code>null</code> if null input. Empty array if an empty array passed
     * in.
     * @since 2.1
     */
     ...
     * </p>
     * <p>
     * This method is the counter part of <code>primitiveToWrapper()</code>. If the passed in class is a wrapper class for a primitive type, this primitive type will be returned (e.g.
     * <code>Integer.TYPE</code> for <code>Integer.class</code>). For other classes, or if the parameter is <b>null</b>, the return value is <b>null</b>.
     * </p>
     *
     ...
     *
     * @param classes the class array to convert, may be null or empty
     * @return an array which contains for each given class, the primitive class or <b>null</b> if the original class is not a wrapper class. <code>null</code> if null input. Empty array if an empty
     * array passed in.
    ...
    /**
     * Returns the class represented by <code>className</code> using the <code>classLoader</code>. This implementation supports the syntaxes "<code>java.util.Map.Entry[]</code>",
     * "<code>java.util.Map$Entry[]</code>", "<code>[Ljava.util.Map.Entry;</code>", and "<code>[Ljava.util.Map$Entry;</code>".
     *
    ...
    public static Class<?> getClass(final ClassLoader classLoader, final String className, final boolean initialize) throws ClassNotFoundException {
        try {
            ...
            if (lastDotIndex != -1) {
                try {
                    return ClassUtils
                        .getClass(classLoader, className.substring(0, lastDotIndex) + ClassUtils.INNER_CLASS_SEPARATOR_CHAR + className.substring(lastDotIndex + 1), initialize);
    ...
    /**
     * Returns the (initialized) class represented by <code>className</code> using the <code>classLoader</code>. This implementation supports the syntaxes "<code>java.util.Map.Entry[]</code>",
     * "<code>java.util.Map$Entry[]</code>", "<code>[Ljava.util.Map.Entry;</code>", and "<code>[Ljava.util.Map$Entry;</code>".
     *
     * @param classLoader the class loader to use to load the class
     * @param className the class name
    ...
    /**
     * Returns the (initialized) class represented by <code>className</code> using the current thread's context class loader. This implementation supports the syntaxes
     * "<code>java.util.Map.Entry[]</code>", "<code>java.util.Map$Entry[]</code>", "<code>[Ljava.util.Map.Entry;</code>", and "<code>[Ljava.util.Map$Entry;</code>".
     *
     * @param className the class name
    ...
    /**
     * Returns the class represented by <code>className</code> using the current thread's context class loader. This implementation supports the syntaxes "<code>java.util.Map.Entry[]</code>",
     * "<code>java.util.Map$Entry[]</code>", "<code>[Ljava.util.Map.Entry;</code>", and "<code>[Ljava.util.Map$Entry;</code>".
     *
     * @param className the class name
     * @param initialize whether the class must be initialized
    ...
    /**
     * <p>
     * Returns the desired Method much like <code>Class.getMethod</code>, however it ensures that the returned Method is from a public class or interface and not from an anonymous inner class. This
     * means that the Method is invokable and doesn't fall foul of Java bug <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4071957">4071957</a>).
    ...
    /**
     * <p>
     * Converts an array of <code>Object</code> in to an array of <code>Class</code> objects. If any of these objects is null, a null element will be inserted into the array.
     * </p>
     * <p>
    ...
    /**
     * 获取方法的标识代码，在不考虑其所属类的情况下。 格式为：<code>&lt;修饰符&gt;&nbsp;&lt;返回值&gt;&nbsp;&lt;类名&gt;.&lt;方法名&gt;(&lt;参数签名列表&gt;)</code>
     */
    ...
    public static <T> T newInstance(ClassLoader classLoader, String clazzName) {
        try {
            Class<?> newClazz = classLoader.loadClass(clazzName);
