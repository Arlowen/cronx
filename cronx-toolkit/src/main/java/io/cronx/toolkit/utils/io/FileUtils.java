 ...
 */
package io.cronx.toolkit.utils.io;
...
import io.cronx.toolkit.utils.io.reader.ReversedLinesFileReader;
...
public class FileUtils {
     ...
     * @throws IOException in case of an I/O error
     */
    public static List<String> reversedReadLines(File file, int maxLines) throws IOException {
        try (ReversedLinesFileReader fileReader = new ReversedLinesFileReader(file, StandardCharsets.UTF_8)) {
            int count = 0;
            List<String> tailStrList = new ArrayList<>();
            String line;
            while (true) {
                if ((line = fileReader.readLine()) == null) {
                    break;
                }
                tailStrList.add(line);
                if (count >= maxLines) {
                    break;
                }
                count++;
    ...
    public static File toFile(URL file) {
        if (file == null) {
            return null;
        }
        try {
    ...
    public static File toFile(URI file) {
        if (file == null) {
            return null;
        }
        try {
