...
@Service
@Slf4j
public class ApiConfigHelperImpl implements ApiConfigHelper {
    ...
    @Override
    public List<ApiKvBaseConfigDO> collectConfigs(Object instance, Long apiSourceId) {
        List<ApiKvBaseConfigDO> configs = new ArrayList<>();
        collectConfigs(instance, apiSourceId, instance.getClass(), configs);
        return configs;
    }

    protected void collectConfigs(Object instance, Long apiSourceId, Class clazz, List<ApiKvBaseConfigDO> configs) {
        try {
            ...
            for (Field field : fields) {
                ...
                ApiKvBaseConfigDO configDO = genConfigDo(configDef, val, apiSourceId);

                configs.add(configDO);
            }

            if (clazz.getSuperclass() != null && clazz.getSuperclass() != Object.class) {
                collectConfigs(instance, apiSourceId, clazz.getSuperclass(), configs);
            }
        } catch (Exception e) {
    ...
    protected ApiKvBaseConfigDO genConfigDo(ConfigDef configDef, String val, Long apiSourceId) {
        ApiKvBaseConfigDO configDO = new ApiKvBaseConfigDO();
        configDO.setConfigName(configDef.name());
        configDO.setConfigValue(val);
        configDO.setApiSourceId(apiSourceId);
